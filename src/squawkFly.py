...
def delete_session():
    ...
    if session is not None and session != '':
        p_session = "sessions/" + session
        if os.path.exists(p_session):
            print("Delete:", p_session)
            shutil.rmtree(p_session)
        else:
            print("Session does not exist:", p_session)
...
def submit(*args):
    ...
    if videos:
        ...
        os.system("python _trace.py " + args_trace1)
        os.system("python _trace.py " + args_trace2)
    ...
    print(p_clip)
    os.system('xdg-open "%s"' % p_clip + '/graphs')
...
calib1.set('../res/camera1_calibration.mp4')
clip1.set('../res/camera1_freekick.mp4')
...
button_del_session = ttk.Button(frame, text="Delete current session", command=delete_session)
button_del_session.grid(column=4, row=8, sticky=(W, E))
# STATIC LABELS
