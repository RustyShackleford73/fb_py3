...
folder = os.path.dirname(infilename)
outfilename = None
try:
...
with open(os.path.join(folder, 'tracer_stats.txt')) as tracerfile:
    tracerstats = tracerfile.read()
    tracerfile.close()

tracerstats = tracerstats.split('\n')
speed = tracerstats[0]
speed_string = "Speed: " + str(speed) + "mph"
...
# annotate curve with movement and speed
trans = ax.get_xaxis_transform()
ann1 = ax.annotate(string, xy=(8.1, 0.3), xycoords=trans)
ann2 = ax.annotate(speed_string, xy=(8.1, 0.15), xycoords=trans)
